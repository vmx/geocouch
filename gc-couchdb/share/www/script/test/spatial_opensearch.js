...
couchTests.spatial_geometry = function(debug) {
  ...
  bbox = [0, 4, 180, 90];
  xhr = CouchDB.request("GET", url_pre + "basicIndex?bbox=" + bbox.join(","));
  TEquals(0, JSON.parse(xhr.responseText).rows.length,
  ...
  var geoJsonDocs = [{"_id": "geoPoint", "geom": { "type": "Point", "coordinates": [105.0, 5.0] }},
  ...
  TEquals(true, /geoPoint/.test(extract_ids(xhr.responseText)),
          ...
          " return the geoPoint");
  TEquals(1, JSON.parse(xhr.responseText).rows.length);
  ...
  TEquals(7, JSON.parse(xhr.responseText).rows.length);
  ...
  TEquals(2, JSON.parse(xhr.responseText).rows.length);
  ...
  TEquals(3, JSON.parse(xhr.responseText).rows.length);
